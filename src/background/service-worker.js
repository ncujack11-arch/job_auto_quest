  ...
  switch (msg.type) {
    ...
    case 'AF_LEARN_SAVE':
      AS.storage.getActiveProfile().then(async (profile) => {
        ...
        try {
          const r = await AS.learnSave.saveWithRollback(profile, msg.items, { force: !!msg.force, sourceHost });
          LOG.info('bg', 'learn saved', r);
          sendResponse(r);
        } catch (e) {
          LOG.error('bg', 'learn save failed, rolled back', e);
